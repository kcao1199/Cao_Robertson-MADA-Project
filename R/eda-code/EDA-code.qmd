...
library(here)
knitr::read_chunk(here("./R/eda-code/edacode.R"))
```
...
The race and ethnicity variables show a normal distribution.
```{r, RACEETHK}
...
The Insurance status variable shows that the majority of the observations came from households with private insurance only. This is mirrored in the facility variable, where a majority of its observation s show private facilities being the location. 
