...
(**Rachel redundantly added code that does the same thing. We can choose our favorite of the two next time we meet.**)
I will now tune the penalty parameter using cross validation to find the penalty with the best roc_auc.
```{r}
lasso_penalty <- logistic_reg(penalty = tune(), mixture = 1) %>%
  set_engine("glmnet")

folds <- rsample::vfold_cv(train_data, v = 5)
tune_grid <- tibble(penalty = 10^seq(-2, -1, length.out = 10))

wf <- workflow() %>%
  add_recipe(lasso_rec)%>%
  add_model(lasso_penalty)
...
# Cross Validation
Now we will compare the two lasso models with the penalty parameters of 0.005 and 0.01 to determine which model performs best when presented with "new data".
