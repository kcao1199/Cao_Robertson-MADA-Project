...
```
The residual plot shows clear collinearity that is not represented by the current model. To find the predictors with collinnearity we will use a correlation matrix. This is because the VIF analysis will not run when multicollinearity is too high.
```{r}
